...
export default function FormConfiguracion({ estudiante, carreras, relojVinculado = false }: FormConfiguracionProps) {
    ...
    const handleRelojSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        ...
        <div className="max-w-3xl mx-auto space-y-8">
            ...
            <div className="space-y-4">
                ...
                <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden transition-all duration-300">
                    <button
                        type="button"
                        onClick={() => setOpenSection(openSection === "smartwatch" ? "" as any : "smartwatch")}
                        className="w-full flex items-center justify-between p-6 sm:p-8 text-left hover:bg-gray-50/50 transition-colors focus:outline-none cursor-pointer"
                    >
                        <div className="flex items-center gap-4">
                            <div className="p-3 bg-teal-50 text-teal-600 rounded-2xl">
                        ...
                        <ChevronDown className={`w-5 h-5 text-gray-400 transition-transform duration-300 ${openSection === "smartwatch" ? "rotate-180" : ""}`} />
                    </button>

                    <div className={`transition-all duration-300 ease-in-out overflow-hidden ${openSection === "smartwatch"
                            ? "max-h-[800px] opacity-100 border-t border-gray-100"
                            : "max-h-0 opacity-0 pointer-events-none"
                        }`}>
