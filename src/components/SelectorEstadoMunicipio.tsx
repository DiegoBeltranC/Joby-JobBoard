...
interface SelectorEstadoMunicipioProps {
    ...
    /** Clases del contenedor (por defecto un grid de 2 columnas en md+). */
    className?: string;
    /** Clases extra para el popover (p. ej. z-index dentro de un modal). */
    contentClassName?: string;
    /** Alineación del popover (Radix), por defecto "center". */
        ...
        <div className={className}>
            ...
            <div className="flex flex-col space-y-1.5">
                ...
                <Popover open={openEstado} onOpenChange={setOpenEstado}>
                    <PopoverTrigger asChild>
                        <Button type="button" variant="outline" role="combobox" aria-expanded={openEstado} className={cn("w-full justify-between bg-white font-normal", !estado && "text-muted-foreground", errorEstado && "border-red-500")}>
                            {estado || "Buscar estado..."} <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                        </Button>
                    </PopoverTrigger>
                    <PopoverContent align={align} className={cn("w-[300px] p-0", contentClassName)}><Command><CommandInput placeholder="Escribe tu estado..." /><CommandList><CommandEmpty>No se encontró el estado.</CommandEmpty><CommandGroup>
            ...
            <div className="flex flex-col space-y-1.5">
                ...
                <Popover open={openMunicipio} onOpenChange={setOpenMunicipio}>
                    <PopoverTrigger asChild>
                        <Button type="button" variant="outline" role="combobox" aria-expanded={openMunicipio} disabled={!estado} className={cn("w-full justify-between bg-white font-normal", !municipio && "text-muted-foreground", !estado && "bg-gray-100", errorMunicipio && "border-red-500")}>
                            {municipio || (estado ? "Buscar municipio..." : "Primero elige un estado")} <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                        </Button>
                    </PopoverTrigger>
                    <PopoverContent align={align} className={cn("w-[300px] p-0", contentClassName)}><Command><CommandInput placeholder="Escribe tu municipio..." /><CommandList><CommandEmpty>No se encontró el municipio.</CommandEmpty><CommandGroup>
